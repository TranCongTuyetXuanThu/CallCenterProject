...
opt = Config()
opt.load("D:\LSTM\infer\infer\opt.log")

model = AudioOnly_v2(opt)
model.to("cpu")
model.load_state_dict(torch.load("D:\LSTM\infer\infer\checkpoint_0_0.pt", map_location=torch.device("cpu"))["state_dict_network"])

wav_path = "D:/LSTM/YAF_back_happy.wav"
...
with torch.no_grad():
    ...
    print( index2label[torch.argmax(output)])
